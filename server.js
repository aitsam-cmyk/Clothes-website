...
// ===== CLOUDINARY IMPORTS & CONFIGURATION START =====
const cloudinary = require('cloudinary').v2;
const { CloudinaryStorage } = require('multer-storage-cloudinary');

// Cloudinary Configuration (.env file se values lega)
cloudinary.config({
...
// Ye abhi local folder ki jagah Cloudinary use karega
// @ts-ignore - Ignore TypeScript warning for params (folder is valid in Cloudinary but not in type definitions)
const storage = new CloudinaryStorage({
  cloudinary: cloudinary,
  params: {
    folder: 'clothes_shop', // Cloudinary dashboard mein is naam ka folder banega
...
if (usePg) {
  ...
  // @ts-ignore - SQLite3 database module doesn't have full TypeScript support
  db = require('./database');
}
...
// Ye line ab upar wali Cloudinary Storage use karegi
...
app.post('/api/products', requireAdmin, upload.array('images'), (req, res) => {
    const { name, price, desc, image_url } = req.body;
    const files = req.files;
    let imageUrl = image_url || 'https://via.placeholder.com/400';
    
    // Cloudinary use kar rahe hain to 'path' mein full URL aata hai
...
app.delete('/api/products/:id', requireAdmin, (req, res) => {
    ...
    if (usePg) {
        ...
        db.get("SELECT COUNT(*) AS cnt FROM order_items WHERE product_id = ?", [id], (e, row) => {
            ...
            db.all("SELECT url FROM product_images WHERE product_id = ?", [id], (err, rows) => {
                if (!err && rows) {
                    rows.forEach(row => imageUrls.push(row.url));
                }
                // @ts-ignore
                db.get("SELECT image_url FROM products WHERE id = ?", [id], (err, row) => {
                    if (!err && row?.image_url) {
...
// Cloudinary direct upload endpoint for admin
app.post('/api/admin/cloudinary/upload', requireAdmin, upload.single('image'), (req, res) => {
    try {
        if (!req.file) {
...
// Cloudinary direct delete endpoint for admin
app.delete('/api/admin/cloudinary/delete', requireAdmin, (req, res) => {
...
// Get Cloudinary images list for admin
app.get('/api/admin/cloudinary/images', requireAdmin, (req, res) => {
    const { folder = 'clothes_shop' } = req.query;
...
// --- FIXED: CHANGED FROM /api/products TO /api/orders ---
...
app.post('/api/orders', upload.single('screenshot'), async (req, res) => {
    ...
    // Ab screenshot bhi Cloudinary par jayegi
